...
export function moduleSource(fragment){
 ...
 if(panels.length!==6||modules.some(([id],i)=>panels[i].getAttribute('data-module-panel')!==id||panels[i].getAttribute('data-module-state')!==(id==='01'?'ready':'pending')))throw new Error('报告模块结构不完整，请联系管理员核验。');
 return panels[0];
}
export function mountModules(meta,panel){
 ...
 for(const [id,title] of modules){
  const button=el('button','module-button'+(id==='01'?'':' pending'));
  ...
  if(id!=='01')button.append(el('small','module-status','即将上线'));
